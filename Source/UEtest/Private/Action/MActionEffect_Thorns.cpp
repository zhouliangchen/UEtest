...
void UMActionEffect_Thorns::StartAction_Implementation(AActor* Instigator)
{
	...
	UMAttributeComponent* AttributeComp = UMAttributeComponent::GetAttributeComp(GetOwnerComp()->GetOwner());
	if(AttributeComp)
	{
...
{
	AActor* OwnerActor = GetOwnerComp()->GetOwner();
	if (!Delta || InstigatorActor == OwnerActor)return;
